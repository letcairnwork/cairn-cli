 ...
 */
export function createWorkspace(path = join(homedir(), 'cairn')) {
...
- \`quests/\` - Your projects (e.g., "Launch My App")
...
\`\`\`bash
cairn create quest "My First Project"
\`\`\`

Or create manually:
- Create folder: \`quests/my-first-project/\`
...
\`\`\`
"Help me create a quest called Launch My App"
\`\`\`
 ...
 */
export function workspaceExists(path = join(homedir(), 'cairn')) {
  return existsSync(join(path, 'quests'));
}
...
export function validateWorkspace(path) {
  const requiredFolders = ['quests', 'inbox'];
  const missing = [];
