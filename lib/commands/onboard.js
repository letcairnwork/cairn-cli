...
export default async function onboard(options) {
  ...
  // Step 1: Prompt for workspace path (if not provided)
  ...
  if (!agentName) {
    const { confirmedName } = await inquirer.prompt([{
      type: 'input',
      name: 'confirmedName',
      message: 'What should we call you (for task assignments)?',
      default: process.env.USER || 'agent'
    }]);
    agentName = confirmedName;
  }
  ...
  try {
    ...
    switch (agentType) {
      ...
      case 'generic':
      default:
        await setupGeneric(setupConfig);
        setupSpinner.stop();
