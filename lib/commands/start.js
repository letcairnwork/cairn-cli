...
import { findTaskBySlug, updateTaskStatus, getTimestamp, getAgentName } from '../utils/task-helpers.js';

export default async function start(taskSlug, options) {
  ...
  if (!taskSlug) {
    ...
    console.log(chalk.dim('Usage:'), chalk.cyan('cairn start <task-slug>'));
    console.log(chalk.dim('Example:'), chalk.cyan('cairn start implement-auth'));
    process.exit(1);
  }
  ...
  try {
    ...
    console.log(chalk.green('✓'), `Started task: ${chalk.cyan(taskSlug)}`);
    console.log(chalk.dim(`  Project: ${task.project}`));
    console.log(chalk.dim(`  Status: in_progress`));
  } catch (error) {
    console.error(chalk.red('Error:'), `Failed to start task: ${error.message}`);
    process.exit(1);
  }
}
